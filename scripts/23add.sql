...
sv.SUBSERVICE_NAME AS 'Название услуги',
    SUM(IFNULL(TBL.priem_all, 0)+IFNULL(TBL.iss_all, 0)+IFNULL(TBL.iss_all_part, 0)+IFNULL(TBL.cons_all, 0)+IFNULL(TBL.priem_cons_all, 0)) AS 'Итого обращений',
    ...
    SUM(IFNULL(TBL.cons_fl, 0)+IFNULL(TBL.priem_cons_fl, 0)) AS 'Всего консультаций Физ',    
    ...
    SUM(IFNULL(TBL.cons_ul, 0)+IFNULL(TBL.priem_cons_ul, 0)) AS 'Всего консультаций Юр'
    FROM ais_service sv
LEFT JOIN sper_standart_type TB ON TB.id = sv.SERVICE_LEVEL
    ...
    LEFT JOIN (SELECT ds.CLASSIFIC_SUBSERVICE_ID, 
        ...
        COUNT(DISTINCT(CASE WHEN ds.main_status_id = 23 AND (ds.app_type_id in (0, 2) OR ds.app_type_id IS NULL) AND ds.CLASSIFIC_SUBSERVICE_ID NOT IN (852, 853, 854, 848, 943) THEN  CONCAT(ds.mainId, '_', ds.DEAL_ID) ELSE NULL END)) AS priem_fl,
        COUNT(DISTINCT(CASE WHEN ds.main_status_id = 23 AND ds.app_type_id not in (0, 2) AND ds.CLASSIFIC_SUBSERVICE_ID NOT IN (852, 853, 854, 848, 943) THEN  CONCAT(ds.mainId, '_', ds.DEAL_ID) ELSE NULL END)) AS priem_ul,
        COUNT(DISTINCT(CASE WHEN ds.main_status_id = 23 AND ds.CLASSIFIC_SUBSERVICE_ID NOT IN (852, 853, 854, 848, 943) THEN CONCAT(ds.mainId, '_', ds.DEAL_ID) ELSE NULL END)) AS priem_all,
...
COUNT(DISTINCT CASE WHEN ds.main_status_id = 25 AND ds.app_type_id not in (0, 2) AND ds.CLASSIFIC_SUBSERVICE_ID NOT IN (852, 853, 854) THEN CONCAT(ds.mainId, '_', ds.CONSULT_ID) ELSE NULL END) AS cons_ul,
        COUNT(DISTINCT(CASE WHEN ds.main_status_id = 23 AND (ds.app_type_id in (0, 2) OR ds.app_type_id IS NULL) AND ds.CLASSIFIC_SUBSERVICE_ID IN (848, 943) THEN  CONCAT(ds.mainId, '_', ds.DEAL_ID) ELSE NULL END)) AS priem_cons_fl,
        COUNT(DISTINCT(CASE WHEN ds.main_status_id = 23 AND ds.app_type_id not in (0, 2) AND ds.CLASSIFIC_SUBSERVICE_ID IN (848, 943) THEN CONCAT(ds.mainId, '_', ds.DEAL_ID) ELSE NULL END)) AS priem_cons_ul,
	COUNT(DISTINCT(CASE WHEN ds.main_status_id = 23 AND ds.CLASSIFIC_SUBSERVICE_ID IN (848, 943) THEN CONCAT(ds.mainId, '_', ds.DEAL_ID) ELSE NULL END)) AS priem_cons_all,
